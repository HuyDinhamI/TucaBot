...
class CustomEnsembleRetriever(BaseRetriever):
    ...
    def weighted_reciprocal_rank(
        ...
        max_index = self.weights.index(max_value)
        anchor_ids = [doc.page_content for doc in doc_lists[max_index]]
        new_doc_lists = []
        for doc in doc_lists:
            temp_docs = []
            for d in doc:
                if d.page_content in anchor_ids:
                    temp_docs.append(d)
            new_doc_lists.append(temp_docs)

        # Associate each doc's content with its RRF score for later sorting by it
        # Duplicated contents across retrievers are collapsed & scored cumulatively
        rrf_score: Dict[str, float] = defaultdict(float)
        for doc_list, weight in zip(new_doc_lists, self.weights):
        ...
        all_docs = chain.from_iterable(new_doc_lists)
        sorted_docs = sorted(
            unique_by_key(
                all_docs,
